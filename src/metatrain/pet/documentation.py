...
class ModelHypers(TypedDict):
    ...
    """Path to a checkpoint for a pretrained :class:`EdgeCompositionModel
    <metatrain.experimental.edge_composition.EdgeCompositionModel>`, whose
    contribution is removed from any atom-pair (``sample_kind == "atom_pair"``)
    targets, analogous to how the (per-atom) composition model baselines per-atom
    targets. Unlike the composition model, this is not fit here: train it separately
    (as its own, standalone ``experimental.edge_composition`` architecture) and point
    this hyper at the resulting checkpoint. ``None`` (the default) disables it."""
