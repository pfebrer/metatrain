...
def couple_tensor_blocks(
    ...
    for key, block in tensor.items():
        o3_lambda_1, o3_lambda_2 = key.values[:2]
        ...
        for o3_lambda, coupled_array in zip(allowed_o3_lambdas, coupled_arrays):
            ...
            new_key = (
                ...
                *key.values[
                    4:
                ],  # the other keys present, not including sigma_1 and sigma_2
                o3_lambda_1,
                o3_lambda_2,
            )
            assert new_key not in key_values
...
def batch_neighborlist(
    ...
    for i_system, system in enumerate(systems):
        ...
        for key, mask in masks.items():
            ...
            system_samples = torch.full(
                (samples_values.shape[0], 1), i_system, device=samples_values.device
            )
...
def radial_to_spherical_harmonics(
    ...
    if rank == 1:
        tmap = uncouple_tensor_blocks(
            tmap, dense_cg_coeffs.to(device=tmap.device, dtype=tmap.dtype)
        )
...
def get_radial_training_transform(nl_options, model, missing_values=0.0):
    ...
    def transform(systems, targets, extra_data):
        ...
        for target_name, target_tmap in targets.items():
            target_tmap = match_samples_to_neighborlist_and_layout(
                target_tmap,
