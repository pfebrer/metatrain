...
def _compute_sparse_properties(layout: TensorMap) -> dict[str, tuple[Labels, torch.Tensor]]:
    ...
    for key, block in layout.items():
        k = [v.item() for v in key.values]
        padded_properties = union_properties[tuple(k[i] for i in non_type_indices)]
        
        sparse_properties[str(k)] = (
            block.properties,
            block.properties.select(padded_properties)
        )
...
def _sparsify_per_atom_atomic_basis_target(
    ...
    unique_types = torch.unique(atom_types_batch).to(torch.int64)
    unique_types: list[int] = unique_types.cpu().tolist() 
    ...
    for key, block in tensor.items():
        for atom_type in unique_types:
            new_key = torch.cat([key.values, torch.tensor([atom_type], device=key.values.device)], dim=0)

            # Key to look up at the sparse_properties dict.
            k = str(new_key.tolist())
            if k not in sparse_properties:
                # This irreps do not exist for this given atom type
                continue
                
            # Get the unpadded properties and the mask to select them from the padded block
